...
HXLINE( 116)		loadButton->screenCenter(::flixel::util::FlxAxes_obj::X_dyn());
...
HXLINE( 123)		saveButton->screenCenter(::flixel::util::FlxAxes_obj::X_dyn());
...
HXDLIN( 283)					if (_this->keyManager->checkStatus(13,_this->status)) {
...
HXDLIN( 292)			if (_this->keyManager->checkStatus(27,_this->status)) {
...
HXDLIN( 298)			if (_this1->keyManager->checkStatus(16,_this1->status)) {
...
HXDLIN( 300)			if (_this2->keyManager->checkStatus(37,_this2->status)) {
...
HXDLIN( 304)			if (_this3->keyManager->checkStatus(39,_this3->status)) {
...
HXDLIN( 308)			if (_this4->keyManager->checkStatus(38,_this4->status)) {
...
HXDLIN( 312)			if (_this5->keyManager->checkStatus(40,_this5->status)) {
...
HXDLIN( 317)			if (_this6->keyManager->checkStatus(32,_this6->status)) {
...
HXLINE( 363)					::String cutName = this->_file->name.substr(0,(this->_file->name.length - 5));
HXLINE( 364)					::haxe::Log_obj::trace((HX_("Successfully loaded file: ",84,24,61,ed) + cutName),::hx::SourceInfo(HX_("source/editors/MenuCharacterEditorState.hx",87,da,fd,31),364,HX_("editors.MenuCharacterEditorState",a2,06,38,48),HX_("onLoadComplete",be,4c,20,63)));
